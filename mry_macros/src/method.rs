    ...
    fn adds_mock_function() {
        ...
        assert_eq!(
            ...
            quote! {
                fn meow(&self, count: usize) -> String {
                    #[cfg(debug_assertions)]
                    if let Some(out) = self.mry.record_call_and_find_mock_output(std::any::Any::type_id(&Self::meow), "Cat::meow", (count.clone())) {
                        return out;
                    }
                    "meow".repeat(count)
                }
    ...
    fn empty_args() {
        ...
        assert_eq!(
            ...
            quote! {
                fn meow(&self, ) -> String {
                    #[cfg(debug_assertions)]
                    if let Some(out) = self.mry.record_call_and_find_mock_output(std::any::Any::type_id(&Self::meow), "Cat::meow", ()) {
                        return out;
                    }
                    "meow".into()
                }
    ...
    fn multiple_args() {
        ...
        assert_eq!(
            ...
            quote! {
                fn meow(&self, base: String, count: usize) -> String {
                    #[cfg(debug_assertions)]
                    if let Some(out) = self.mry.record_call_and_find_mock_output(std::any::Any::type_id(&Self::meow), "Cat::meow", (base.clone(), count.clone())) {
                        return out;
                    }
                    base.repeat(count)
                }
                ...
                pub fn mock_meow<'mry>(&'mry mut self, base: impl Into<mry::Matcher<String>>, count: impl Into<mry::Matcher<usize>>) -> mry::MockLocator<'mry, (String, usize), String, mry::Behavior2<(String, usize), String> > {
                    mry::MockLocator {
                        ...
                        key: std::any::Any::type_id(&Self::meow),
                        name: "Cat::meow",
                        matcher: Some((base.into(), count.into(),).into()),
                        _phantom: Default::default(),
                    }
                }
            }
    ...
    fn input_reference_and_str() {
        ...
        assert_eq!(
            ...
            quote! {
                fn meow(&self, out: &'static mut String, base: &str, count: &usize) -> () {
                    #[cfg(debug_assertions)]
                    if let Some(out) = self.mry.record_call_and_find_mock_output(std::any::Any::type_id(&Self::meow), "Cat::meow", (out.clone(), base.to_string(), count.clone())) {
                        return out;
                    }
                    *out = base.repeat(count);
                }
                ...
                pub fn mock_meow<'mry>(&'mry mut self, out: impl Into <mry::Matcher<String>>, base: impl Into<mry::Matcher<String>>, count: impl Into<mry::Matcher<usize>>)
                    ...
                    mry::MockLocator {
                        ...
                        key: std::any::Any::type_id(&Self::meow),
                        name: "Cat::meow",
                        matcher: Some((out.into(), base.into(), count.into(),).into()),
                        _phantom: Default::default(),
                    }
                }
            }
            ...
            quote! {
                ...
                pub fn mock_meow<'mry>(&'mry mut self, arg0: impl Into<mry::Matcher<A>>, count: impl Into<mry::Matcher<usize>>, arg2: impl Into<mry::Matcher<String>>) -> mry::MockLocator<'mry, (A, usize, String), String, mry::Behavior3<(A, usize, String), String> > {
                    mry::MockLocator {
                        ...
                        key: std::any::Any::type_id(&Self::meow),
                        name: "Cat::meow",
                        matcher: Some((arg0.into(), count.into(), arg2.into(),).into()),
                        _phantom: Default::default(),
                    }
                }
            }
